...
import { ArrowLeft, Calendar, Clock, Languages, Tag } from 'lucide-react'
import { getAllPosts, getPost, getTranslation, type Locale } from '@/lib/blog'
...
export function staticParams(locale: Locale) {
...
function hrefFor(slug: string, locale: Locale): string {
  return locale === 'ar' ? `/blog/ar/${slug}/` : `/blog/${slug}/`
}

/** Shared server view for EN and localized article pages. */
export function ArticleView({ slug, locale }: ArticlePageProps) {
  const post = getPost(slug, locale)
  if (!post) notFound()
  const { meta, html, toc } = post
  const translation = getTranslation(slug, locale)
  const rtl = locale === 'ar'
  const promptPath = rtl ? `ar/${slug}` : slug
  ...
  const articleSchema = {
    ...
    '@type': 'BlogPosting',
    '@id': `${BASE}${hrefFor(slug, locale)}#article`,
    headline: meta.title,
    description: meta.description,
    datePublished: meta.date,
    dateModified: meta.date,
    inLanguage: rtl ? 'ar' : 'en-US',
    wordCount: words,
    timeRequired: `PT${readingTime}M`,
    keywords: meta.tags.join(', '),
    articleSection: meta.tags[0] ?? 'Infrastructure',
    image: `${BASE}${hrefFor(slug, locale)}og.png`,
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `${BASE}${hrefFor(slug, locale)}`,
    },
    author: {
      '@type': 'Person',
      '@id': `${BASE}/#person`,
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: meta.title, item: `${BASE}${hrefFor(slug, locale)}` },
    ],
  }
    ...
    <>
      ...
      >
        ...
        <div className="mt-6 font-mono text-[13px] flex flex-wrap items-center gap-1.5">
          ...
          <span className="text-muted-foreground">$</span>
          <span className="text-foreground">cat {promptPath}.md</span>
        </div>
        ...
        <header className="mt-6 pb-6 border-b border-primary/15">
          <h1
            ...
            style={rtl ? { fontFamily: 'var(--font-arabic), sans-serif' } : undefined}
            itemProp="headline"
          >
            {meta.title}
          </h1>
          <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-[12px] font-mono text-muted-foreground">
            ...
            {/* translate button: active when a translation exists, greyed out otherwise */}
            {translation ? (
              <Link
                href={hrefFor(translation.slug, translation.locale)}
                className="flex items-center gap-1.5 text-primary hover:text-foreground transition-colors"
                title={rtl ? 'Read in English' : 'اقرأ المقال بالدارجة'}
              >
...
export function articleMetadata(slug: string, locale: Locale): Metadata {
  ...
  const rtl = locale === 'ar'
  const ogImage = {
    url: `${hrefFor(slug, locale)}og.png`,
    width: 1200,
    height: 630,
    alt: meta.title,
  }
